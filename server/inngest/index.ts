...
export const inngest = new Inngest({ id: "grocery-store" });
...
const sendMonthlyOffers = inngest.createFunction(
  {
    ...
    triggers: [{ cron: "0 0 1 * *" }], // Run on the 1st of every month
  },
  async ({ step }) => {
    // Implementation for sending monthly offers
    const { deals, users } = await step.run(
      ...
      async () => {
        ...
        // Implementation for fetching deals and users
    ...
    if (deals.length === 0 || deals.length === 0) {
      return { skipped: true, reason: "No deals or users found" };
    }
    let sentCount = 0;
    // send in batches of 10 to avoid hitting email rate limits
    const batchSize = 10;
...
const autoAssignRider = inngest.createFunction(
  {
    ...
    name: "Auto Assign Rider Orders",
    triggers: [{ event: "order/placed" }],
  },
  async ({ event, step }) => {
    const { orderId } = event.data;
    // wait for 5 mins
    await step.sleep("await-5-minutes", "5m");
    const result = await step.run("assign-rider", async () => {
      ...
      // generate 6 digit OTP for rider to confirm delivery
      ...
      history.push({
        status: "Assigned",
        timestamp: new Date(),
        note: `Order assigned to rider ${availableRider.name}`,
      });
